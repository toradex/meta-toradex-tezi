...
uboot_prep_kimage() {
	if [ -e arch/${ARCH}/boot/compressed/vmlinux ]; then
		...
		linux_suffix=""
		linux_comp="none"
	elif [ -e arch/${ARCH}/boot/Image.gz ]; then
		rm -f linux.bin
		cp -l arch/${ARCH}/boot/Image.gz linux.bin.gz
		vmlinux_path=""
		linux_suffix=".gz"
		linux_comp="gzip"
...
fitimage_assemble() {
	...
	if [ "x${ramdiskcount}" = "x1" ] ; then
		...
		for img in squashfs cpio.lz4 cpio.lzo cpio.lzma cpio.xz cpio.gz ext2.gz cpio; do
